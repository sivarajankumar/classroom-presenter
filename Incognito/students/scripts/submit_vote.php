<?php
	...
	function submitVote($type, $vote_count, $id, $db_conn)
	{
	...
	function undoVote($type, $vote_count, $id, $db_conn)
	{
		$vote_count = $vote_count - 1;
	...
	function processVote($type, $id, $username, $vote)
	{
		$db_conn = connectToDB();
		...
		if ( $type == 'Q' )
		{
			...
			{
				// submit a vote
				submitVote($type, $vote_count, $id, $db_conn);
			}
			else
			{
				// undo a vote submission
				undoVote($type, $vote_count, $id, $db_conn);
			}
	...
	processVote($type, $id, $username, $vote);
?>
